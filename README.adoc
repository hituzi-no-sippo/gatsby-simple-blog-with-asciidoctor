...
:!email: {author-email}
:revnumber: v1.23.0
:revdate: 2020-01-08T12:42:28+0900
:revremark: Update link to gatsby-transformer-asciidoc to v0.11.0
:doctype: article
...
==== Asciidoc in `_includes` directory are not create nodes

The `ignore` option in link:https://github.com/gatsbyjs/gatsby/tree/master/packages/gatsby-source-filesystem[
gatsby-source-filesystem^] is used so that the node of asciidoc in `_includes`
