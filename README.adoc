...
:!email: {author-email}
:revnumber: v1.26.3
:revdate: 2020-01-09T22:41:27+0900
:revremark: fix description of the case where edit link is not displayed.
:doctype: article
...
.Cases
* `siteMetadata.articles.dir` is `''`.
* Field of `siteMetadata.articles.filePath` doesn't exist.
* `siteMetadata.articles.isOtherRepositroy` is `false` and
  `siteMetadata.repository.url` is `''`.
* Match with `siteMetadata.articles.ignore` option.
...
Used `siteMetadata.repository`.
Doesn't displays link, if `url` is `''` or `displaysLink` is `false`.
