...
:!email: {author-email}
:revnumber: v1.56.0
:revdate: 2020-01-20T17:35:41+0900
:revremark: Add the following logs: + \
            * add url into disqus config + \
            * change prop type from `any` to `string` in `Disqus` component + \
            * Change the default display setting of disqus from non \
              display to display + \
            * add accessory informations of article component + \
            * add link for comment counts of disqus
:doctype: article
...
* Link for comment count of disqus. (close link:{issues-url}/133[#133^])
